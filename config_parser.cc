...
int NginxConfig::getPortNumber() {

    // We only accept port numbers right now, so there should be a single
    // statement containing a server block
    if (statements_.size() != 1) {
        return -1;
    }

    std::shared_ptr<NginxConfigStatement> statement = statements_[0];
    if (statement->tokens_.size() != 1 ||
        statement->tokens_[0] != SERVER_TOKEN ||
        statement->child_block_ == NULL) {
        return -1;
    }

    // The child of the server token should be an NginxConfig containing
    // just a port number.;
    if (statement->child_block_->statements_.size() != 1) {
        return -1;
    }

    // The first token should be "listen"
    statement = statement->child_block_->statements_[0];
    if (statement->tokens_.size() != 2 ||
        statement->tokens_[0] != PORT_TOKEN) {
        return -1;
    }

    // The second token should be an integer followed by a semicolon.
    std::string port_string = statement->tokens_[1];
    for (unsigned i = 0; i < port_string.size(); i++) {
        if (!std::isdigit(port_string[i])) {
            return -1;
        }
    }

    // Finally, return the port number...
